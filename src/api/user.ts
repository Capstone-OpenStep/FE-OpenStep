...
export interface getAcheivementResponse {
  isSuccess: boolean;
  code: string;
  message: string;
  result: Acheievement[];
...
export const getAcheievement = async (): Promise<Acheievement[]> => {
  const response = await api.get<getAcheivementResponse>(`/member/achievements`);
  return response.data.result;
};